...
% ut_plot_lens_distortion(camparameters,[nrow,ncol])
...
[C,h]=contour(mx,my,dr,0.5:0.5:5,'k');
if isempty(C)
    [C,h]=contour(mx,my,dr,0.05:0.01:0.5,'k');
    clabel(C,h);
else
    clabel(C,h);
    [C,h]=contour(mx,my,dr,5:1:10,'k');
    if ~isempty(C)
        clabel(C,h);
        [C,h]=contour(mx,my,dr,10:2:20,'k');
        if ~isempty(C)
            clabel(C,h);
            [C,h]=contour(mx,my,dr,20:4:40,'k');
            if ~isempty(C)
                clabel(C,h);
            end
        end
