function [Cmean,Cgaussian,Dir1,Dir2,Lambda1,Lambda2]=patchcurvature(FV,usethird,smoothSteps)
...
%   smoothSteps : The amounts of steps for smoothing Lambda1 and Lambda2
%
% outputs,
...
tic
for it=1:smoothSteps
    Lambda1D = Lambda1;
    Lambda2D = Lambda2;
    for i=1:nv
        Lambda1(i) = (Lambda1D(i) + mean(Lambda1D(Ne{i})))/2;
        Lambda2(i) = (Lambda2D(i) + mean(Lambda2D(Ne{i})))/2;
    end
end
toc

% enforce than min<max
I = find(Lambda1<Lambda2);
Lambda1T = Lambda1; Dir1T = Dir1;
Lambda1(I) = Lambda2(I); Lambda2(I) = Lambda1T(I);
Dir1(I,:) = Dir2(I,:); Dir2(I,:) = Dir1T(I,:);
